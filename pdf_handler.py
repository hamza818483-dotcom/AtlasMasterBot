...
async def update_dashboard(msg, data: dict):
    pct = data.get('pct', 0); bar_len = 20
    filled = int(bar_len * pct / 100); bar = '█' * filled + '░' * (bar_len - filled)
    ...
    for attempt in range(10):
        ...
        try:
            ...
            if exp_row:
                ...
                exp = custom_text if (mode == 'custom' and custom_text) else mcq.get('explanation','')[:200]
                if tag_name: exp = f"{exp}\n{tag_name}" if exp else tag_name
            else: exp = mcq.get('explanation','')[:200]
...
async def send_polls_to_channel(update, context, channel_id, mcqs, topic, mood, images=None):
    ...
    if mood == "image" and images:
        per_image = max(1, total // len(images)); mcq_idx = 0
        for i, img_data in enumerate(images):
            ...
            page_mcqs = mcqs[mcq_idx:mcq_idx + per_image]; first_pid, psent = None, 0
        ...
        pre_text = get_pre_message(topic, total); pre_msg = await bot.send_message(chat_id=channel_id, text=pre_text)
    ...
    if query: await query.message.reply_text(f"✅ {total} polls sent!")
...
async def process_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE, is_qbm: bool = False, mood: str = 'topic'):
    ...
    all_mcqs = []; page_links = {}; sent_count = 0
    if is_qbm:
